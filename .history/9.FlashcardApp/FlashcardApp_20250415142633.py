import json
import random 
import os

#Loading Flashcards 
def load_flashcards(filename):
    if not os.path.exists(filename):
        print(f"[INFO] '{filename}' not found. Creating a new one.")
        with open(filename, 'w') as f:
            json.dump([], f)  # start with empty list of flashcards

    with open(filename, 'r') as f:
        return json.load(f)
    
#Saving Flashcards
def save_flashcards(flashcards,filename):
    with open(filename,'w') as f:
        json.dump(flashcards,f)

# Step 3: Quiz the user
def take_quiz(flashcards):
    if not flashcards:
        print("No flashcards to quiz yet!")
        return
    
    random.shuffle(flashcards)
    for card in flashcards:
        answer = input(f"Q: {card['questions']} \nYour answer: ")
        if answer.strip().lower() == card['answer'].strip().lower():
            print("✅ Correct!\n")
        else:
            print(f"❌ Nope! The answer is: {card['answer']}\n")


    #Adding Flashcard
def add_flashcard(flashcards):
    question = input('Enter Question: ')
    answer = input('Enter Answer: ')
    flashcards.append({'questions': question, 'answer': answer})
    print("Flashcard Added!\n")

    #Menu 
def main():
    filename = 'flashcards.json'
    flashcards = load_flashcards(filename)

    while True:
        print("   -- Flashcard Quiz App ---")
        print("1. Take a quiz")
        print("2. Add a flashcard")
        print("3. Save flashcards")
        print("4. Exit")

        choice = input('Enter Choice!: ')

        if choice.isdigit():
            choice = int(choice)
            if choice == 1:
                take_quiz(flashcards)
            elif choice == 2:
                add_flashcard(flashcards)
            elif choice == 3:
                save_flashcards(flashcards,filename)
            elif choice == 4:
                print('Goodbye!')
                break
            else:
                print("Invalid Choice!")

if __name__ == "__main__":
    main()

                
        
